...
FILENAME="DLT_issues_all.csv"
echo "WORKING WITH $BASEURL AND FILENAME $FILENAME AND PROJECT $PROJECT"

# Calculating the Total Pages after enabling Pagination
TOTALPAGES=`curl -I -i -u $USERNAME:$TOKEN -H "Accept: application/vnd.github.v3+json" -s ${BASEURL}\?state=all | grep -i link: 2>/dev/null|sed 's/link: //g'|awk -F',' -v  ORS='\n' '{ for (i = 1; i <= NF; i++) print $i }'|grep -i last|awk '{print $1}' | tr -d '\<\>' | tr '\?\&' ' '|awk '{print $3}'| tr -d '=;page'`

i=1
echo is it $TOTALPAGES totalpages
until [ $i -gt $TOTALPAGES ]
do
  result=`curl -s -u $USERNAME:$TOKEN -H 'Accept: application/vnd.github.v3+json' ${BASEURL}\?state=all\&page=${i} 2>&1`
  echo $result > tempfile
  cat tempfile| jq '['\"${PROJECT}\"'] + (.[] | [.id, .state, .created_at])| @csv' | tr -d '\\"' >> $FILENAME
  ((i=$i+1))
done
